...
drawmenu(void)
{
	...
	if (lines > 0) {
		...
		for (item = curr; item != next; item = item->right, i++)
			drawitem(
				item,
				(!draw_input && prompt && *prompt) ? (x + ((i / lines) *  (mw / columns))) - mw : (x + ((i / lines) *  ((mw - x) / columns))) - promptw,
				y + (((i % lines) + 1) * bh),
				mw / columns
			);
	} else if (matches) {
...
usage(void)
{
	die("usage: dmenu [-bcfPsv] [-noi] [-g columns] [-l lines] [-p prompt] [-fn font] [-m monitor]\n"
